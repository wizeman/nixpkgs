  ...
  config = mkIf cfg.enable {
    systemd.services.transmission = {
      ...
      serviceConfig.ExecStartPre = ''
          ${pkgs.stdenv.shell} -c "mkdir -p ${homeDir} ${settingsDir} ${fullSettings.download-dir} ${fullSettings.incomplete-dir} && chmod 775 ${fullSettings.download-dir} ${fullSettings.incomplete-dir} && chmod 700 ${homeDir} ${settingsDir} && rm -f ${settingsDir}/settings.json && cp -f ${settingsFile} ${settingsDir}/settings.json"
      '';
