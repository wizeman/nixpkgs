...
let
  ...
  # beware, the hashes of patches from Arch's URLs change from time to time
  gcc46Patch = fetchurl {
  ...
  libpng16Patch = fetchurl {
    url = "http://sources.gentoo.org/cgi-bin/viewvc.cgi/gentoo-x86/sci-astronomy/celestia/files/celestia-1.6.1-libpng16.patch?revision=1.1";
...
stdenv.mkDerivation {
  ...
  # something very strange is going on, as this was OK in master
  NIX_CFLAGS_LINK = "-lgtk-x11-2.0 -lgtkglext-x11-1.0 -lcairo -lpango-1.0 -lgobject-2.0 -lglib-2.0 -lgdk_pixbuf-2.0";
